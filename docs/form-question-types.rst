...
The ``search`` appearance allows the enumerator to filter the list of available choices based on their label. The filtering is case-insensitive and matches any subset of the label text.
...
.. tip::
  ...
  To prevent intersecting segments, use the :func:`intersects` function. By default, constraint violation messages are shown after leaving the question screen. Add ``incremental=true``` in the ``parameters`` column to instead show the constraint message in a toast immediately when the constraint is violated.
