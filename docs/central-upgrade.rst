...
-----------------------------
There are several time-consuming steps in this upgrade. Central will not be accessible during those steps. If you have several thousands of versions of a single form (generally generated by an automated script that uses the API to update a form attachment), please email support@getodk.org with details about your infrastructure and the number of form versions you have **before starting the upgrade process** so we can help you plan for a successful upgrade.

If you have several tens of thousands of submissions, consider temporarily increasing server performance and :ref:`memory allocation <central-install-custom-memory>` before upgrading.


.. danger::

  If you start the upgrade and find that it takes too long, in many cases the only way to safely bring the server back will be to recover from backup. You should always back your server up before starting an upgrade and it is especially important with this version.
